...
fn get_manhattan_distance_by_id(database:&Database, first:String, second:String) {
    match database {
        Database::SimpleMovies{url} => {
            ...
            println!("SimpleMovies Manhattan Distance between id {} with name {} and id {} with name {} is {}", first, first_user.name, second, second_user.name, distance);
        },
        Database::Books{url} => {
...
fn get_euclidean_distance_by_id(database:&Database, first:String, second:String) {
    match database {
        Database::SimpleMovies{url} => {
            ...
            println!("SimpleMovies Euclidean Distance between id {} with name {} and id {} with name {} is {}", first, first_user.name, second, second_user.name, distance);
        },
        Database::Books{url} => {
            let manager = BookDBManager::connect_to(&url);
            let engine = Engine::<i32,String> {phantom_U: PhantomData, phantom_I: PhantomData};
        ...
        Database::SimpleMovies{url} => {
            ...
            for first_user in &users_named_first {
                ...
                for second_user in &users_named_second {
                    ...
                    println!("SimpleMovies Minkowski Distance with grade {} between {} and {} is {}", grade, first_user.name, second_user.name, distance);
                }
            }
        },
        Database::Books{url} => {
...
fn get_minkowski_distance_by_id(database:&Database, first:String, second:String, grade:i32) {
    match database {
        Database::SimpleMovies{url} => {
            ...
            println!("SimpleMovies Minkowski Distance with grade {} between id {} with name {} and id {} with name {} is {}", grade, first, first_user.name, second, second_user.name, distance);
        },
        Database::Books{url} => {
            let manager = BookDBManager::connect_to(&url);
            let engine = Engine::<i32,String> {phantom_U: PhantomData, phantom_I: PhantomData};
